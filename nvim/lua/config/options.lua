...
--  ================================================================================================
--  TITLE : options.lua 
--  ABOUT : settings for nvim
--  LINKS : - 
--  FILES : - 
--  TAGS  : [[nvim]] [[editor]] [[system]]
--  ================================================================================================
...
--  ---[ Basic settings   ]----------------------------------------------------------------------///
    ...
    --  set.numberwidth = 6     --  number of columns used for the line number
    set.cursorcolumn = true     --  highlight the screen column of the cursor
    set.cursorline = true       --  highlight the screen line of the cursor
    --  set.spelllang = { "en", "nl" }      --  language(s) to do spell checking for
    --  set.spell = true
    set.wrap = true         --  lines wrap and continue on the next line
    set.linebreak = true    --  wrap long lines at a blank
    set.showbreak = "└ "    --  string to use at the start of wrapped lines
    set.breakindent = true  --  wrapped line repeats indent
    set.breakindentopt = "shift:4"      --  settings for 'breakindent'
    set.scrolloff = 12      --  minimum nr. of lines above and below cursor
    --  set.sidescrolloff   --  min. nr. of columns to left and right of cursor
    set.cmdheight = 0       --  number of lines to use for the command-line
--  ---------------------------------------------------------------------------------------------\\\
--  ---[ Tabbing / Indentation ]-----------------------------------------------------------------///
    ...
    set.smartindent = true    --  smart autoindenting for C programs
    set.expandtab = true      --  use spaces when <Tab> is inserted
    set.grepprg = "rg, --vimgrep"     --  program to use for ":grep"
    set.grepformat = "%f:%1:%c:%m"    --  format of 'grepprg' output
--  ---------------------------------------------------------------------------------------------\\\
--  ---[ Search Settings  ]----------------------------------------------------------------------///
...
--  ---[ Visual Settings  ]----------------------------------------------------------------------///
    ...
    --  set.colorcolumn = "4,38,62,100"    --  columns to highlight
    set.showmatch = true        --  briefly jump to matching bracket if insert one
    ...
    set.lazyredraw = false      --  don't redraw while executing macros
    set.redrawtime = 10000      --  timeout for 'hlsearch' and |:match| highlighting
    set.maxmempattern = 20000   --  maximum memory (in Kbyte) used for pattern search
    set.synmaxcol = 300         --  maximum column to find syntax items
    set.background = "dark"     --  "dark" or "light", used for highlight colors
    set.statusline = "%R%=%f   %P   %l/%c  "  --  custom format for the status line
--  ---------------------------------------------------------------------------------------------\\\
--  ---[ File Handling    ]----------------------------------------------------------------------///
    ...
    set.ttimeoutlen = 0        --  time out time for key codes in milliseconds
    set.autoread = true        --  autom. read file when changed outside of Vim
    set.autowrite = false      --  automatically write file if changed
    set.diffopt:append("vertical")              --  options for using diff mode
    set.diffopt:append("algorithm:patience")    --  options for using diff mode
    set.diffopt:append("linematch:60")          --  options for using diff mode
--  ---------------------------------------------------------------------------------------------\\\
--  ---[ Undo Directory   ]----------------------------------------------------------------------///
    local undodir = "~/.local/share/nvim/undodir"    -- Undo directory path
    set.undodir = vim.fn.expand(undodir)               -- Expand to full path
...
--  ---[ Behavior Settings ]---------------------------------------------------------------------///
    ...
    set.clipboard:append("unnamedplus")   --  use the clipboard as the unnamed register
    set.modifiable = true    --  changes to the text are not possible
    set.encoding = "UTF-8"   --  encoding used internally
    set.wildmenu = true      --  use menu for command line completion
...
--  ---[ Fold Settings    ]----------------------------------------------------------------------///
    ...
    --    set.foldexpr = "v:lua.vim.treesitter.foldexpr()"    --  expression used when 'foldmethod' is "expr"
    set.foldlevel = 0             --  close folds with a level higher than this
    set.foldtext = ""

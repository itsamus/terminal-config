    ...
    selection=$(command fzf -i \
                       ...
                       case "$MIME" in
                                text/*) batcat --color=always --line-range=:500 {} ;;
                                 audio/*) echo "" && exiftool -Title {} && echo "" && exiftool -Duration -Artist {} && echo "========================================"  && ffmpeg -hide_banner -loglevel error -i {} -map 0:v -c copy -f image2pipe - | chafa - ${FZF_PREVIEW_COLUMNS}x${FZF_PREVIEW_LINES} --format=symbols --colors=truecolor {} ;;
                                image/*) chafa -s ${FZF_PREVIEW_COLUMNS}x${FZF_PREVIEW_LINES} --format=symbols --colors=truecolor {} ;;
                               application/pdf) mutool draw -F stext "{}" 2>/dev/null | python3 /home/itsamus/Downloads/clean_pdf.py ;;
